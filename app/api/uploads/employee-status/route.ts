// File: app/api/uploads/employee-status/route.ts
import { NextResponse, NextRequest } from "next/server";
import prisma from "@/lib/prisma";
import { getSession } from "@/lib/session";
import * as xlsx from "xlsx";

// Interface untuk baris Excel, sesuaikan dengan header
interface EmployeeStatusExcelRow {
  Year: number;
  Month: number | string;
  Company: string;
  Permanent: number;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows: EmployeeStatusExcelRow[] = xlsx.utils.sheet_to_json(sheet, {
      raw: false,
    });
    ...
    const dataToUpsert = rows
      .map((row) => {
        ...
        return {
          ...
          month: monthNumber,
          companyId: companyId,
          permanentCount: Number(row.Permanent) || 0,
          contractCount: Number(row.Contract) || 0,
        };
      })
    ...
    return NextResponse.json({
      message: `${dataToUpsert.length} baris data status karyawan berhasil diimpor.`,
    });
  } catch (error) {
    console.error("API Error in /uploads/employee-status:", error);
    return NextResponse.json(
