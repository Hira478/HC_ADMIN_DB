// components/forms/OrganizationStructureForm.tsx
"use client";
import { Company } from "@/types";
import React, { useState } from "react";

const CHART_INDICATORS = [
  { id: "risikoTataKelola", label: "Risiko & Tata Kelola" },
  { id: "sdmUmum", label: "SDM & Umum" },
...
const CARD_INDICATORS = [
  { id: "enablerCount", label: "Enabler Count", type: "number" },
  {
    id: "revenueGeneratorCount",
    label: "Revenue Generator Count",
    type: "number",
  },
  { id: "divisionCount", label: "Division Count", type: "number" },
  { id: "departmentCount", label: "Department Count", type: "number" },
  { id: "avgSpanDepartment", label: "AVG Span Dept", type: "number" },
  { id: "avgSpanEmployee", label: "AVG Span Employee", type: "number" },
];
...
export const OrganizationStructureForm: React.FC<FormProps> = ({
  companies,
}) => {
  ...
  const [formData, setFormData] = useState({
    ...
    ...Object.fromEntries(
      [...CHART_INDICATORS, ...CARD_INDICATORS].map((i) => [i.id, ""])
    ),
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setMessage("");
    try {
      const res = await fetch("/api/input/organization-structure", {
        method: "POST",
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          {isLoading ? "Menyimpan..." : "Simpan Data Struktur Organisasi"}
        </button>
      </div>
      {message && (
        <p
